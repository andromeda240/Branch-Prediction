...
#
# Zero size entries are ignored.
# Duplicate entries (same LOW_PC and SIZE) are output only once.
#
...
def run_cmd_and_parse(binary, output_filename):
    ...
    seenEntries = set()
    for id, entry in subprograms_dict.items():
        t = (entry.low_pc, entry.size)
        if t not in seenEntries:
            entry.dump(out)
            seenEntries.add(t)
    out.close()
    return True
