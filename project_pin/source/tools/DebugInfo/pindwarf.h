...
typedef enum {
    PINDWARF_ENUMERATE_ALL,
    PINDWARF_ENUMERATE_NO_INLINES,
    PINDWARF_ENUMERATE_ONLY_INLINES
} PINDWARF_ENUMERATE_SUBPROGRAMS_MODE;
...
/*!
  ...
  @param[out] subprorams_list  A list of SubprogramInfo holding the dwarf information of the subprograms in the image.
                               ...
                               @note the strings pointed to by short_name and linkage_name are owned by the pindwarf library
                                     and are valid as long as the process exists.
                                     The caller must not attempt to free these strings.
                               @note the caller is responsible for freeing the allocated array using 
                                     @ref DeallocateSubprogramsList().
  @param[in] mode              The enumeration mode.
...
PINDWARF_RES GetSubprogramsListInImage(const char* path,
                                       unsigned* list_size,
                                       SubprogramInfo** subprorams_list,
                                       PINDWARF_ENUMERATE_SUBPROGRAMS_MODE mode) __attribute__ ((visibility ("default")));
