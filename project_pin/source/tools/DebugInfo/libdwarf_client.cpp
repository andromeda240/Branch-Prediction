...
{
    static std::set< std::pair< Dwarf_Addr, Dwarf_Addr > > printedRanges;
    Dwarf_Error error;
    Dwarf_Half tag = 0;
    int res;
    ...
    // Avoid printing duplicate ranges
    std::pair< Dwarf_Addr, Dwarf_Addr > range(lowPC, highPC);
    if (printedRanges.find(range) != printedRanges.end())
    {
        return TRUE;
    }
    printedRanges.insert(range);
