...
static void ReplayImageEntry()
{
    ...
    switch (tag)
    {
        case 'L':
        {
            ...
            ///NOTE: Taking the client lock is no longer necessary for IMG_ReplayImageLoad
            ///      but is still necessary for RTN_CreateAt.
            PIN_LockClient();
...
static VOID PrintRTNs(IMG img)
{
    for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
    {
        ...
        for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn))
        {
            ...
            // We make sure RTN Open works when callback is called from replay
            // even when replay is done from analysis routine
